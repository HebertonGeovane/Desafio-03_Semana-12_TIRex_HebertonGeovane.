...
import { useCart } from "../../../../components/cartcontext/cartcontext"; // Importe o hook do contexto do carrinho
...
const AsgaardSection: React.FC = () => {
  const { addToCart, updateQuantity } = useCart();; // Utilize o hook do carrinho
  const [quantity, setQuantity] = useState(1); // Controle da quantidade local para o componente

  const handleAddToCart = () => {
    // Defina os dados do produto, que podem vir de um banco de dados ou props
    const product = {
      id: 1,
      name: "Asgaard Sofa",
      price: "Rs. 250,000.00",
  ...
  const handleQuantityChange = (action: "increase" | "decrease") => {
    const newQuantity = action === "increase" ? quantity + 1 : Math.max(1, quantity - 1);
    setQuantity(newQuantity);
    updateQuantity(1, newQuantity); // Atualiza a quantidade no carrinho com o produto ID 1
  };
        ...
        <div className="w-[606px] h-[730px] flex flex-col space-y-5">
          ...
          <div>
            ...
            <div className="flex space-x-2 mt-2">
              <button className="w-[30px] h-[30px] rounded bg-[#B88E2F] text-black">L</button>
              <button className="w-[30px] h-[30px] rounded bg-[#F9F1E7] text-[#000000]">XL</button>
              <button className="w-[30px] h-[30px] rounded bg-[#F9F1E7] text-[#000000]">XS</button>
            </div>
          </div>
